...
// ==========================================
const RESTAURANTE_NAME = "Tradición D'Pilitas";
const RESTAURANTE_SLOGAN = "La Pastelería • Tortas, Postres & Bocaditos";
const WHATSAPP_NUMBER = "51900000000"; // Reemplaza con tu número de WhatsApp con código de país
const FACEBOOK_URL = "https://facebook.com/TradicionDPilitas";
const MAPS_URL = "https://www.google.com/maps/search/?api=1&query=Tradicion+DPilitas+La+Pasteleria";
const LOGO_FOOTER_PATH = "/logo.png"; // Ruta del logo de Tradición D'Pilitas
const BANNER_PATH = "/logo.png"; // Ruta del banner principal
const MARQUEE_TEXT = "🎂 ¡BIENVENIDOS A TRADICIÓN D'PILITAS! • TORTAS ARTESANALES, POSTRES Y BOCADITOS DULCES • HAZ TU PEDIDO POR WHATSAPP • ";
// ==========================================
...
export default function App() {
  ...
  return (
    <div className="max-w-md mx-auto bg-white min-h-screen relative shadow-2xl overflow-hidden flex flex-col font-sans">
      <header className="sticky top-0 bg-white/95 backdrop-blur-md z-50 px-5 py-3 flex justify-between items-center border-b border-rose-100 shadow-sm">
        <div className="flex items-center gap-3">
          <img src={LOGO_FOOTER_PATH} alt={RESTAURANTE_NAME} className="w-12 h-12 object-contain drop-shadow-sm" />
        ...
        <div className="flex items-center gap-2">
          ...
          >
            ...
            {cartCount > 0 && (
              <span className="absolute -top-0.5 -right-0.5 min-w-[18px] h-4 bg-secondary text-white rounded-full text-[9px] font-bold flex items-center justify-center px-1">
                {cartCount}
              </span>
            )}
          </motion.div>
      ...
      <div className="px-5 pt-4">
        <motion.button 
          ...
          className="w-full bg-gradient-to-r from-pink-500 via-primary to-rose-600 text-white py-3 px-4 rounded-2xl flex items-center justify-center gap-2 font-bold text-[10px] sm:text-[11px] uppercase tracking-wide border border-rose-300 relative overflow-hidden group text-center shadow-md"
        >
          ...
          <span>¡Celebra tu cumpleaños en Tradición D'Pilitas! 🎂 <span className="text-yellow-200 font-black underline">Regístrate aquí</span> y recibe una porción de postre de regalo 🎁</span>
        </motion.button>
      </div>

      <div className="px-5 pt-4 pb-3">
        <div className="relative w-full rounded-3xl overflow-hidden shadow-xl aspect-[2/1] bg-gradient-to-br from-rose-50 via-white to-blue-50 flex items-center justify-center p-4 border border-rose-200/60">
          <img src={BANNER_PATH} alt="Tradición D'Pilitas Logo Banner" className="h-full max-h-[140px] w-auto object-contain drop-shadow-lg transform hover:scale-105 transition-transform duration-300" />
        </div>
      </div>
      ...
      <main className="flex-1 overflow-y-auto pb-32 px-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              {cat.items.map((dish, idx) => (
                <motion.div
                  key={idx}
                  whileHover={{ y: -4 }}
                  className="bg-white rounded-[2rem] overflow-hidden flex flex-col shadow-sm border border-rose-100 hover:border-primary/30 hover:shadow-md transition-all duration-200"
                >
                  <div className="bg-gradient-to-b from-rose-50/50 to-white aspect-square flex items-center justify-center relative overflow-hidden p-3 border-b border-rose-50">
                    <img 
                      src={dish.imagen || LOGO_FOOTER_PATH} 
                      alt={dish.nombre}
                      onClick={() => setSelectedImage(dish.imagen || LOGO_FOOTER_PATH)}
                      className="w-full h-full object-contain drop-shadow-sm hover:scale-105 transition-transform duration-300 cursor-pointer"
                    />
                  </div>
                  ...
                  <div className="p-4 flex flex-col flex-1">
                    ...
                    {dish.descripcion && (
                      <p className="text-[10px] text-gray-500 leading-tight mb-2 line-clamp-3">
                        {dish.descripcion}
                      </p>
                    )}
                    ...
                    <div className="flex items-center justify-between mt-2">
                      ...
                      <motion.button
                        ...
                        className="w-8 h-8 bg-primary/10 rounded-full flex items-center justify-center text-primary transition-colors duration-200 shrink-0 hover:bg-primary hover:text-white"
                      >
        ...
        <section className="mt-8 mb-4 border border-rose-100 bg-rose-50/40 rounded-3xl p-5 text-center shadow-sm">
          <h3 className="font-title text-primary text-[22px] leading-tight mb-2">¿Cómo estuvo tu experiencia?</h3>
          <p className="text-[11px] text-gray-600 mb-4 px-4">Califica nuestros postres y servicio para seguir ofreciéndote lo mejor</p>
          <motion.button 
            whileTap={{ scale: 0.95 }}
            onClick={() => setShowReviewForm(true)}
            className="bg-primary text-white px-6 py-3 rounded-2xl font-bold text-sm shadow-md shadow-primary/20 flex items-center justify-center gap-2 mx-auto w-full hover:bg-rose-700 transition-colors"
          >
            <Star size={18} className="fill-white" />
            Reseña nuestro servicio y postres
          </motion.button>
        </section>

        <footer className="mt-8 pt-8 pb-10 border-t border-rose-100 flex flex-col items-center justify-center">
          <img src={LOGO_FOOTER_PATH} alt={RESTAURANTE_NAME} className="w-32 h-32 object-contain mb-3 drop-shadow-md" />
          <p className="font-title text-2xl text-primary mb-1">{RESTAURANTE_NAME}</p>
          <p className="font-slogan text-xs text-secondary font-bold mb-4">{RESTAURANTE_SLOGAN}</p>
          <p className="text-[11px] text-gray-400 font-medium">© 2026 Tradición D'Pilitas. Todos los derechos reservados.</p>
        </footer>
